...
export class IndexToColor {

    public indexToRGB(colorindex: number): string {
    ...
    public RGBToColor(color: string): string {
        if (color == "null")
            return "RGB出错！"
        // RGB颜色值的正则
        //var reg = /^#([0-9a-fA-f]{3}|[0-9a-fA-f]{6})$/;
        var reg: RegExp = /^(rgb|RGB)/;
        if (reg.test(color)) {
            let strHex: string = "#";
            // 把RGB的3个数值变成数组
            let colorArr: string[] = color.replace(/(?:\(|\)|rgb|RGB)*/g, "").split(",");
            // 转成16进制
            for (let i = 0; i < colorArr.length; i++) {
                let hex: string = Number(colorArr[i]).toString(16);
                if (hex === "0") {
                    hex += hex;
                }
